"""Structural tests for the apply-tax-clearance-south-africa scaffold.
...
from server import check_compliance_readiness, get_status  # noqa: E402


class TestScaffoldHonesty:
    async def test_get_status_reports_scaffold(self):
        s = await get_status()
        assert s["skill"] == "apply-tax-clearance-south-africa"
    ...
    async def test_stub_returns_structured_not_implemented(self):
        out = await check_compliance_readiness()
...
def test_mcp_server_starts():
    ...
    assert mcp.name == "mia-apply-tax-clearance-south-africa"
