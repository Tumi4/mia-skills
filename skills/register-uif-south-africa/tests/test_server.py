"""Structural tests for the register-uif-south-africa scaffold.
...
from server import check_registration_obligation, get_status  # noqa: E402


class TestScaffoldHonesty:
    async def test_get_status_reports_scaffold(self):
        s = await get_status()
        assert s["skill"] == "register-uif-south-africa"
    ...
    async def test_stub_returns_structured_not_implemented(self):
        out = await check_registration_obligation()
...
def test_mcp_server_starts():
    ...
    assert mcp.name == "mia-register-uif-south-africa"
