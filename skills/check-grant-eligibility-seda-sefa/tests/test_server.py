"""Structural tests for the check-grant-eligibility-seda-sefa scaffold.
...
from server import get_status, list_programmes  # noqa: E402


class TestScaffoldHonesty:
    async def test_get_status_reports_scaffold(self):
        s = await get_status()
        assert s["skill"] == "check-grant-eligibility-seda-sefa"
...
def test_mcp_server_starts():
    ...
    assert mcp.name == "mia-check-grant-eligibility-seda-sefa"
