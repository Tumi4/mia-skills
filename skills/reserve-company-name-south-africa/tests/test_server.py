"""Structural tests for the reserve-company-name-south-africa scaffold.
...
from server import check_name_availability, get_status  # noqa: E402


class TestScaffoldHonesty:
    async def test_get_status_reports_scaffold(self):
        s = await get_status()
        assert s["skill"] == "reserve-company-name-south-africa"
    ...
    async def test_stub_returns_structured_not_implemented(self):
        out = await check_name_availability()
...
def test_mcp_server_starts():
    ...
    assert mcp.name == "mia-reserve-company-name-south-africa"
