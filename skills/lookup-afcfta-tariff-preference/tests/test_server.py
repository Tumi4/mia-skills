"""Structural tests for the lookup-afcfta-tariff-preference scaffold.
...
from server import get_status, lookup_tariff_preference  # noqa: E402


class TestScaffoldHonesty:
    async def test_get_status_reports_scaffold(self):
        s = await get_status()
        assert s["skill"] == "lookup-afcfta-tariff-preference"
    ...
    async def test_stub_returns_structured_not_implemented(self):
        out = await lookup_tariff_preference()
...
def test_mcp_server_starts():
    ...
    assert mcp.name == "mia-lookup-afcfta-tariff-preference"
